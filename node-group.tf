  ...
  provisioner "local-exec" {
    command = <<EOT
      ...
      INSTANCE_IDS=$(aws ec2 describe-instances \
        --region ${var.region} \
        --filters "Name=tag:eks:nodegroup-name,Values=python-dev-nodegroup" "Name=instance-state-name,Values=running" \
      ...
      for INSTANCE_ID in $INSTANCE_IDS; do
        echo "Setting name for instance $INSTANCE_ID: ${var.cluster_name}-$COUNT"
        aws ec2 create-tags \
          --region ${var.region} \
